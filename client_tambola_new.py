    ...
    for row in ticketGrid:
        for numberBox in row:
            if (not numberBox['text']):
                if(platform.system() == 'Darwin'):
                    numberBox.configure(state = 'disabled', disabledbackground = '#ff8a65',
...
def gameWindow():
    ...
    flashNumberLabel = canvas2.crete_text(400, screen_height/1.3, text = "Wating for other players to join...", font = ("Chalkboard SE", 30), fill = "#3e2723")
...
def askPlayerName():
    ...
    button = Button(nameWindow, text="Save", font=("Chalkboard SE", 30),width=11, command=saveName, height=2, bd=3)
